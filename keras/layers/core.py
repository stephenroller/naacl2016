...
class Merge(Layer):
    def __init__(self, layers, mode='sum', concat_axis=-1, dot_axes=-1):
        ''' Merge the output of a list of layers or containers into a single tensor.
            mode: {'sum', 'mul', 'concat', 'ave', 'join', 'cos', 'dot', 'inner'}
        '''
        if len(layers) < 2:
            raise Exception("Please specify two or more input layers (or containers) to merge")

        if mode not in {'sum', 'mul', 'concat', 'ave', 'join', 'cos', 'dot', 'inner'}:
            raise Exception("Invalid merge mode: " + str(mode))

        if mode in {'sum', 'mul', 'ave', 'cos', 'inner'}:
    ...
    def output_shape(self):
        ...
        elif self.mode == 'cos' or self.mode == 'inner':
