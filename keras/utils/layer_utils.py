...
def container_from_config(original_layer_dict, custom_objects={}):
    ...
    if name == 'Merge':
        ...
        layer_list = []
        for layer in layers:
            init_layer = container_from_config(layer, custom_objects)
            layer_list.append(init_layer)
        merge_layer = Merge(layer_list, mode)
        return merge_layer

    elif name == 'InnerProductMerge':
        left = container_from_config(layer_dict.get('left'), custom_objects)
        right = container_from_config(layer_dict.get('right'), custom_objects)
        return globals()[name](left, right)
        ...
        for node in nodes:
            layer = container_from_config(layer_dict['nodes'].get(node['name']), custom_objects)
            node['layer'] = layer
        ...
        kwargs = {'encoder': container_from_config(layer_dict.get('encoder_config'), custom_objects),
                  'decoder': container_from_config(layer_dict.get('decoder_config'), custom_objects)}
