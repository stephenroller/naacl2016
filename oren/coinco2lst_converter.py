'''
Used to convert the coinco (Kremer 2014) xml dataset format to the format used in LST 2007.
...
def subs2text(subs_element):
    ...
    retval = ';'.join([sub + " " + str(freq) for freq, sub in sorted_subs])
    #retval = unidecode.unidecode(retval.decode('utf-8'))
    return retval
    ...
    for sent in coinco.iter('sent'):
        ...
        for token in tokens.iter('token'):
            ...
            if token.attrib.get('id') != 'XXX' and token.attrib.get('problematic') == 'no':# and (len(token.attrib.get('lemma').strip().split()) == 1):
                tokens_num += 1
#                if '-' in token.attrib.get('wordform'):
#                    print token.attrib.get('wordform') + "\t" + token.attrib.get('lemma')
                try:
                    ...
                    gold_file.write("%s %s :: %s\n" % (target_key, token.attrib.get('id'), subs2text(token.find('substitutions')).encode('utf-8')))
